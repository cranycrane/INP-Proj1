...
async def test_login(dut, uid=''):
    ...
    lcd = lcd.lower()
    assert lcd == "xjerab28", "Invalid output"
...
@tb_test()
async def test_my_moving(dut):
    """Decrement value of the first memory cell, i.e. *ptr--"""
    instcnt, mem, _ = await run_program(dut, '+>>+<<+@', timeout_ns = 100_000)
    assert mem[8] == 2
    assert mem[10] == 1

@tb_test()
async def test_my_while(dut):
    """Decrement value of the first memory cell, i.e. *ptr--"""
    instcnt, mem, _ = await run_program(dut, '+++>+-+++++++++<-@', timeout_ns = 100_000)
    assert mem[19] == 9
    assert mem[18] == 2
...
@tb_test()
async def test_my_comments(dut):
    """Decrement value of the first memory cell, i.e. *ptr--"""
    instcnt, mem, _ = await run_program(dut, '+a@', timeout_ns = 100_000)
    assert mem[3] == 1

@tb_test()
async def test_my_counting(dut):
    """Decrement value of the first memory cell, i.e. *ptr--"""
    instcnt, mem, _ = await run_program(dut, '+++-++@', timeout_ns = 100_000)
    assert mem[7] == 4

@tb_test()
async def test_my_superWhile(dut):
    """Decrement value of the first memory cell, i.e. *ptr--"""
    instcnt, mem, _ = await run_program(dut, '[]@', timeout_ns = 100_000)
    assert mem[7] == 4
